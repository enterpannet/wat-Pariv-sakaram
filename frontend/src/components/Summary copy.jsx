...
const Summary = () => {
    ...
    if (password === '0871955621') { // Replace with the actual password you want to use
      try {
        if (recordToDelete.type === 'income') {
          ...
          setIncome((prevIncome) => prevIncome.filter((item) => item.id !== recordToDelete.id));
        } else {
          await axios.delete(`${import.meta.env.VITE_API_URL}/expenses/${recordToDelete.id}`);
          setExpense((prevExpense) => prevExpense.filter((item) => item.id !== recordToDelete.id));
        }
    ...
    { title: 'รายละเอียด', dataIndex: 'description', key: 'description' },
    {
      ...
      render: (_, record) => (
        <>
          ...
          <Button type="link" danger onClick={() => showDeleteModal({ ...record, type: 'income' })}>ลบ</Button>
        </>
      ),
    },
  ];
    ...
    { title: 'รายละเอียด', dataIndex: 'description', key: 'description' },
    {
      ...
      render: (_, record) => (
        <>
          ...
          <Button type="link" danger onClick={() => showDeleteModal({ ...record, type: 'expenses' })}>ลบ</Button>
        </>
      ),
    },
  ];
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 bg-white rounded-lg shadow-lg">
      ...
      >
        <p>โปรดใส่รหัสผ่านเพื่อลบรายการ:</p>
        <Input.Password
          value={password}
